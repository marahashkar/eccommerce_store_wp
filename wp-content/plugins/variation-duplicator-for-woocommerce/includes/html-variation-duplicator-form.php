			...
			<ul class="wc-radios">
				...
				<li>
					<label>
						...
						<?php esc_html_e( 'Set image to other variation(s)', 'variation-duplicator-for-woocommerce' ) ?>
	...
	<div class="form-field form-row form-row-last">
		...
		<p class="variable-list variable-image-duplicate-from">
			...
			<select id="variation_image_from_<?php echo esc_attr( $variation_id ); ?>" data-allow_clear="true"
					...
					class="variable-image-duplicate-select wc-enhanced-select"
				...
				printf( '<option %2$s value="%1$s">#%1$s %3$s</option>', esc_attr( $parent_id ), disabled( ! has_post_thumbnail( $parent_id ), true, false ), ( ! has_post_thumbnail( $parent_id ) ? esc_html__( ' - have no image to clone', 'variation-duplicator-for-woocommerce' ) : '' ) );
				echo '</optgroup>';
				...
				if ( ! empty( $child_ids ) ) {
					...
					foreach ( $child_ids as $child_id ) {
						...
						printf( '<option %2$s value="%1$s">#%1$s %3$s</option>', esc_attr( $child_id ), disabled( empty( $variation_data['image_id'] ), true, false ), ( empty( $variation_data['image_id'] ) ) ? esc_html__( ' - have no image to clone', 'variation-duplicator-for-woocommerce' ) : '' );
