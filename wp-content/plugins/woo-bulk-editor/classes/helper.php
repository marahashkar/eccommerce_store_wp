...
final class WOOBE_HELPER {
    ...
    public static function get_users($use_cache = 0) {

        if (empty(self::$users)) {
            $users = get_users(array('fields' => array('ID', 'display_name'), 'who' => 'authors'));
    ...
    public static function draw_tooltip($text, $direction = 'down') {
        ?>
        <a class="info_helper zebra_tips1" title="<?= $text ?>"><img src="<?php echo WOOBE_ASSETS_LINK . 'images/info-icon.png' ?>" width="15" alt="" /></a>
        <?php
    }
